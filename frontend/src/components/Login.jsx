...
function Login() {
  ...
  .post(`${import.meta.env.VITE_BACKEND_URL}/users/login`, userInfo)
      .then((res) => {
        ...
        if (res.data) {
          // alert("Loggedin Successfully");// we will use toast
      ...
      // if we don't response we will make catch , in catch we handle error ,we will have error we will handle in catch 
        setAuthUser(res.data.user); // this will store tha data on webrowsers local storage
      }).catch((err) => {

        if (err.response) {
          console.log(err)
          // alert("login Error : " + err.response.data.message);//we will use toast 
          toast.error('login Error : ' + err.response.data.message);
          setTimeout(()=>{
          //we don't have anything we will keep this empty 
          },2000)
